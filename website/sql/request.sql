...
UPDATE `option` SET `name`='nouveau_nom_option', 
       		    `kernel_version`='nouveau_kernel', 
		    `kernel_sub`='nouveau_sub' 
WHERE `id` = id_de_l_option;
DELETE FROM `hardware_option` WHERE `id` = id_de_l_option;
DELETE FROM `option` WHERE `id` = id_de_l_option;

-- recherche de tuples à modifier
-- présence de doublons en cas de recherche peu précise
-- TODO à voir pour corriger
SELECT `option_id`, `hardware_id`, `option`.`name`, `hardware`.`name` 
       `kernel_version`, `kernel_sub`;
FROM `option` JOIN `hardware_option` ON (`option_id` = `option`.`id`)
     	      JOIN `hardware` ON (`hardware_id` = `hardware`.`id`);
WHERE `hardware`.`name` LIKE '%nom_du_materiel%' OR 
      `option`.`name` LIKE '%nom_de_l_option';

-- modification d'un tuple
UPDATE `hardware_option` SET `hardware_id`='nouvel_id_materiel',
       			     `option_id`='nouvel_id_option'
WHERE `hardware_id` = id_du_materiel AND `option_id` = id_de_l_option;

-- suppression d'un tuple
DELETE FROM `hardware_option` 
WHERE `hardware_id` = id_du_materiel AND `option_id` = id_de_l_option;