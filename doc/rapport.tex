...
sujet. Celui-ci nous a tout de suite intéressé puisque c'est avant tout un
...
retours que nous avons eu. Enfin, le codage de l'application et les tests se
...
langages afin de créer le site de notre projet. Le couple PHP et MySQL ont
...
\begin{description}
    \item[make oldconfig :] Qui permet de récupérer la configuration du noyau
        qui s'exécute sur la machine, et séléctionne automatiquement les
...
nous avions envisagé de détecter le matériel sur la machine qui executerait
...
exempe), et construire un fichier \textit{.config} avec les bonnes options pour
...
Au début, nous avions laisser à l'utilisateur la possibilité de résoudre
...
nous étions confronté était qualifié de NP-complet. Cependant suite aux
...
satisfaisabilité. Cependant, ces conditions sont elle même des règles de
...
qu'une solution serait très difficle à trouver, surtout avant l'échéance, nous
...
resolution par \textit{brute-force} et la résolution par \textit{dpll}.


\subsubsection{Résoudre les règles de satisfaisabilité par brute-force}
\label{ssub:Résoudre les règles de satisfaisabilité par brute-force}
Pour résoudre une règle de satisfaisabilité, nous avons d'abord tenté
...
approfondir d'avantage cette solution. De ce fait nous avons stoppé toute
...
grand nombre de résultat pour des critères séparés.\\
...
n'ont pas permis d résoudre les conflits automatiquement ni de générer une
...
trouvée, nous avions le choix entre refaire cette gestion nous-même, ce qui
...
variables d'environnement propres à son Makefile comme par exemple
...
des noeuds auraient comme valeur soit un arbre, soit un opérateur logique (||
...
retourne l'indice du menu oû se trouve l'option. Cette indice atteint au
...
Note : Pour l'utilisation de notre application sur \textit{Windows}, il a fallu
...
testé sur cinq machines différentes.
...
chargements suivant prennent en moyenne deux fois moins de temps.
...
l'application et retrouver l'état des options tel qu'il les avait laissé après
enregistrement.\\
...
L'API \textit{kconfiglib} pourrait par exemple être réecrite en C, avec
...
que la communauté aura créé. Au fil du temps, les données du site s'affineront
pour être de plus en plus précises. On peut également imaginer que d'autres
informations seront modifiables à partir du site, comme par exemple, la
...
options qui ne sont pas modifiable telles que \textit{STRING/HEX/INT} serait
...
nos connaissances informatiques. Nous avons du nous familiariser avec la
